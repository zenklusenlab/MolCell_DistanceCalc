function [cytval,nucval] = RNA_coloc2loc_3D(mask1, mask2, mrna5file, mrna3file, pixelsize, zpixelsize, radius, dist, alchk,nucval,cytval)
...
if(~isempty(mrna5file))
    ...
    if ~isempty(img2)
        ...
        mrna5nuc=mrna5nuc(mrna5nuc(:,5)>=1,1:4);
    else
        mrna5nuc =[];
    end
    
    mrna5cyt=nucleus(mrna5, img1, inten_dist1);
    mrna5cyt=mrna5cyt(mrna5cyt(:,5)>=1,1:4);
end
...
if(alchk==1)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
         if ~isempty(img2)
             vals=[];
             [mrna5_coloc_mrna3_nuc,mrna5_mrna3_coloc_val_nuc,vals]= colocalize2loc_3d(mrna5nuc, mrna3nuc,pixelsize,zpixelsize,radius);
%              mrna5nuc(vals,:) = [];
         else
             [mrna5_coloc_mrna3_nuc] = [];
             [mrna5_mrna3_coloc_val_nuc] = [];
         end
        [mrna5_coloc_mrna3_cyt,mrna5_mrna3_coloc_val_cyt,vals]= colocalize2loc_3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%         mrna5cyt(vals,:) = [];
    ...
    if ~isempty(img2)
        mrna5_mrna3_coloc_val_nuc_fin(:,3:4) =  mrna5_mrna3_coloc_val_nuc(:,3:4);
    else
        mrna5_mrna3_coloc_val_nuc_fin=[];
    end
    
    mrna5_mrna3_coloc_val_cyt_fin(:,3:4) =  mrna5_mrna3_coloc_val_cyt(:,3:4);
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [mrna5_coloc_mrna3_nuc,mincol,vals]= colocalize2loc_3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%             mrna5nuc(vals,:) = [];
        else
            [mrna5_coloc_mrna3_nuc] = [];
        end
%         
        vals=[];
        [mrna5_coloc_mrna3_cyt,mincol,vals]= colocalize2loc_3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize,radius);
%         mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, mrna5_coloc_mrna3_cyt, mrna5nuc, mrna3nuc, mrna5_coloc_mrna3_nuc, pixelsize,img2);
    end
...
if(a>0)
    ...
    mrna5_3Data1(:,2) = spot1cyt(spot1_coloc_spot2_cyt(:,1),4);
    mrna5_3Data1(:,3) = spot2cyt(spot1_coloc_spot2_cyt(:,4),4);
    mrna5_3Data1(:,4) = spot1_coloc_spot2_cyt(:,3);
end
...
csvwrite('Cy5spots.csv',spot1cyt);
csvwrite('Cy3spots.csv',spot2cyt);
...
if ~isempty(img2)
    ...
    if(a>0)
        ...
        mrna5_3Data2(:,4) = spot1_coloc_spot2_nuc(:,3);
    end
