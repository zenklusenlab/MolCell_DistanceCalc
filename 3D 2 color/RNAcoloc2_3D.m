function [cytval,nucval] = RNAcoloc2_3D(mask1, mask2, mrna5file, mrna3file, pixelsize, zpixelsize, radius, dist, alchk,nucval,cytval)
    ...
    mrna5(round(mrna5(:,2))==0,:) = [];
       ...
       for i = 1:length(mrna5)
        D = zeros(size(mrna5,1),1);
      	D(i+1:end) = double((mrna5(i+1:end,1)-mrna5(i,1)).^2 + (mrna5(i+1:end,2)-mrna5(i,2)).^2+ (mrna5(i+1:end,3)-mrna5(i,3)).^2);
        D = sqrt(D);
        D = D*pixelsize;
        out =find(D(i+1:end)<dist);
        if ~isempty(out)
...
if(alchk==1)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
         if ~isempty(img2)
             vals=[];
             [mrna5_coloc_mrna3_nuc,mrna5_mrna3_coloc_val_nuc,vals]= colocalize2_3d(mrna5nuc, mrna3nuc,pixelsize, zpixelsize, radius);
             mrna5nuc(vals,:) = [];
         else
             [mrna5_coloc_mrna3_nuc] = [];
             [mrna5_mrna3_coloc_val_nuc] = [];
         end
        [mrna5_coloc_mrna3_cyt,mrna5_mrna3_coloc_val_cyt,vals]= colocalize2_3d(mrna5cyt, mrna3cyt,pixelsize, zpixelsize, radius);
        mrna5cyt(vals,:) = [];
...
elseif(alchk==0)
    if (~isempty(mrna5file) &&  ~isempty(mrna3file))
        if ~isempty(img2) 
            vals=[];
            [mrna5_coloc_mrna3_nuc,mincol,vals]= colocalize2_3d(mrna5nuc, mrna3nuc,pixelsize, zpixelsize, radius);
%             mrna5nuc(vals,:) = [];
        else
            [mrna5_coloc_mrna3_nuc] = [];
        end
%         
        vals=[];
        [mrna5_coloc_mrna3_cyt,mincol,vals]= colocalize2_3d(mrna5cyt, mrna3cyt,pixelsize,zpixelsize, radius);
%         mrna5cyt(vals,:) = [];
        twospotInput(mrna5cyt, mrna3cyt, mrna5_coloc_mrna3_cyt, mrna5nuc, mrna3nuc, mrna5_coloc_mrna3_nuc, pixelsize,img2);
    end
